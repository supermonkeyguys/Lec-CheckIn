...
import {
  POINTS_PAGE_PATHNAME,
  RECORD_PAGE_PATHNAME,
} from '../../router/router';
...
const ClockStart: FC = React.memo(() => {
  ...
  const { currentTime, isRunning, startTimer, stopTimer } = useTimer();
  ...
  return (
    <>
      <Card className={styles.cardContainer}>
        <ClockCircleOutlined
          style={{ fontSize: '32px', color: 'rgba(22, 119, 255)' }}
        />
        ...
        <div className={styles.displayCheckIn}>
          ...
          />
          <span>
            <strong>本周打卡时长: </strong>
          </span>
          <span style={{ color: Number(percent) >= 100 ? 'green' : 'red' }}>
            {' '}
...
ClockStart.displayName = 'ClockStart';
        ...
        >
          <Row align={'middle'} gutter={16} justify="space-between">
            <Col flex="none">
              <div
                style={{ fontSize: '20px' }}
                className={`${styles.commonBtn} ${styles[`btn${index}`]}`}
              >
                {bi.icon}
              </div>
            </Col>
            <Col flex="auto">
              <Space direction="vertical" size={4}>
                ...
                <span style={{ fontSize: 12, color: '#999' }}>{bi.text}</span>
              </Space>
            </Col>
            <Col>
