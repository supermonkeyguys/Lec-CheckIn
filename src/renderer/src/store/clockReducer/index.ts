...
export const checkInSlice = createSlice({
  ...
  reducers: {
    ...
    updateCurrent: (
      state: CheckInState,
      action: PayloadAction<Partial<CheckInState['current']>>
    ) => {
    ...
    getCheckInData: (
      state: CheckInState,
      action: PayloadAction<{ records: CheckInRecord[]; stats: CheckInStats }>
    ) => {
      state.records = action.payload.records.reduce(
        (acc, record) => {
